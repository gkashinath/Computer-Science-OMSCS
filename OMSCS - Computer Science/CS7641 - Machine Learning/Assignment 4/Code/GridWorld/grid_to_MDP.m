...
%            R( (i-1)*sz+j, (i-1)*sz+j-1, 1 ) = grid(i+1,j)*probab;
            else_p = (1-probab)/(s-1);
        end
        
        if ( j < sz )
            P ( (i-1)*sz+j, (i-1)*sz+j+1, 1 ) = B(2)*else_p;
 %           R( (i-1)*sz+j, (i-1)*sz+j+1, 1 ) = grid(i+1,j+2)*else_p*B(2);
        end
        if ( i > 1 )
            P ( (i-1)*sz+j, (i-2)*sz+j, 1) = B(3)*else_p;
  %          R( (i-1)*sz+j, (i-2)*sz+j, 1 ) = grid(i,j+1)*else_p*B(3);
        end
        if ( i < sz )
            P ((i-1)*sz+j, (i)*sz+j, 1 ) = B(4)*else_p;
   %         R( (i-1)*sz+j, (i)*sz+j, 1 ) = grid(i+2,j+1)*else_p*B(4);
        end
  ...
  %          R( (i-1)*sz+j, (i-1)*sz+j, 2 ) = grid(i+1,j+2)*probab;
            else_p = (1-probab)/(s);
        else
            P( (i-1)*sz+j, (i-1)*sz+j+1, 2 ) = probab;
  %          R( (i-1)*sz+j, (i-1)*sz+j+1, 2 ) = grid(i+1,j+2)*probab;
            else_p = (1-probab)/(s-1);
        end
        
        if ( j > 1 )
            P ( (i-1)*sz+j, (i-1)*sz+j-1, 2 ) = B(1)*else_p;
   %         R( (i-1)*sz+j, (i-1)*sz+j-1, 2 ) = grid(i+1,j)*else_p*B(1);
        end
        if ( i > 1 )
            P ( (i-1)*sz+j, (i-2)*sz+j, 2) = B(3)*else_p;
   %         R( (i-1)*sz+j, (i-2)*sz+j, 2 ) = grid(i,j+1)*else_p*B(3);
        end
        if ( i < sz )
            P ((i-1)*sz+j, (i)*sz+j, 2 ) = B(4)*else_p;
   %         R( (i-1)*sz+j, (i)*sz+j, 2 ) = grid(i+2,j+1)*else_p*B(4);
        end
    ...
    %        R( (i-1)*sz+j, (i-1)*sz+j, 3 ) = grid(i,j+1)*probab;
            else_p = (1-probab)/(s);
        else
            P( (i-1)*sz+j, (i-2)*sz+j, 3 ) = probab;
    %        R( (i-1)*sz+j, (i-2)*sz+j, 3 ) = grid(i,j+1)*probab;
            else_p = (1-probab)/(s-1);
        end
        
        if ( j > 1 )
            P ( (i-1)*sz+j, (i-1)*sz+j-1, 3 ) = B(1)*else_p;
     %       R( (i-1)*sz+j, (i-1)*sz+j-1, 3 ) = grid(i+1,j)*else_p*B(1);
        end
     ...
     %       R( (i-1)*sz+j, (i)*sz+j, 3 ) = grid(i+2,j+1)*else_p*B(4);
        end
      ...
      %      R( (i-1)*sz+j, (i-1)*sz+j, 4 ) = grid(i+2,j+1)*probab;
            else_p = (1-probab)/(s);
        else
            P( (i-1)*sz+j, (i)*sz+j, 4 ) = probab;
      %      R( (i-1)*sz+j, (i)*sz+j, 4 ) = grid(i+2,j+1)*probab;
            else_p = (1-probab)/(s-1);
        end
        
        if ( j > 1 )
            P ( (i-1)*sz+j, (i-1)*sz+j-1, 4 ) = B(1)*else_p;
      %      R( (i-1)*sz+j, (i-1)*sz+j-1, 4 ) = grid(i+1,j)*else_p*B(1);
        end

        if ( j < sz )
            P ( (i-1)*sz+j, (i-1)*sz+j+1, 4 ) = B(2)*else_p;
      %      R( (i-1)*sz+j, (i-1)*sz+j+1, 4 ) = grid(i+1,j+2)*else_p*B(2);
        end

        if ( i > 1 )
            P ( (i-1)*sz+j, (i-2)*sz+j, 4) = B(3)*else_p;
       %     R( (i-1)*sz+j, (i-2)*sz+j, 4 ) = grid(i,j+1)*else_p*B(3);
        end
